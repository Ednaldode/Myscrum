...
// 
let analiseAtrasado = 0, analiseAtencao = 0, analiseAdiantado = 0,
    agendamentoAtrasado = 0, agendamentoAtencao = 0, agendamentoAdiantado = 0, 
    atendimentoAtrasado = 0, atendimentoAtencao = 0, atendimentoAdiantado = 0, 
    quitacaoAtrasado = 0, quitacaoAtencao = 0, quitacaoAdiantado = 0,
    juridicoAtrasado = 0, juridicoAtencao = 0, juridicoAdiantado = 0;
...
$(function(){
  ...
  let contadorJuridico = 0;
  let listaPJuridico = $("#problemasJuridico").attr("title").substr(1, ($("#problemasJuridico").attr("title").length - 2)).split(", ");
  let listaFJuridico = $("#problemasJuridico").text().substr(1, ($("#problemasJuridico").text().length - 2)).split(", ");
  ...
  $.each(teste, function(indice, valor){
    ...
    if ($('#statusProcesso' + $(valor).attr('id')).text() == "Análise"){
      ...
      contadorTermo += 1;
    } else if ($('#statusProcesso' + $(valor).attr('id')).text() == "Jurídico"){
      $('#problemas' + $(valor).attr('id')).text(`${listaFJuridico[contadorJuridico]}/${listaPJuridico[contadorJuridico]}`);
      contadorJuridico += 1;
  ...
  $(".titulos").html(`
    ...
    <span title="Adiantados: ${juridicoAdiantado}\nAvisos: ${juridicoAtencao}\nAtrasados: ${juridicoAtrasado}" class="titulo-child afazer">Jurídico | ${qtdProcessos[4]}</span>
    <span title="Concluído: ${qtdProcessos[5]}" class="titulo-child fazendo">Concluído | ${qtdProcessos[5]}</span>
    <span title="Fora de Garantia: ${qtdProcessos[6].replace("]", "")}" class="titulo-child feito">Fora de Garantia | ${qtdProcessos[6].replace("]", "")}</span>
    `);
