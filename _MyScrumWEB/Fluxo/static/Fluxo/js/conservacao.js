...
// 
let limpezaAtrasado = 0, limpezaAtencao = 0, limpezaAdiantado = 0, agendamentoAtrasado = 0, agendamentoAtencao = 0, agendamentoAdiantado = 0, manutencaoAtrasado = 0, manutencaoAtencao = 0, manutencaoAdiantado = 0, pagarAtrasado = 0, pagarAtencao = 0, pagarAdiantado = 0;
...
$(function(){
  ...
  let listaPLimpeza = $("#problemasLimpeza").attr("title").substr(1, ($("#problemasLimpeza").attr("title").length - 2)).split(", ");
  let listaFLimpeza = $("#problemasLimpeza").text().substr(1, ($("#problemasLimpeza").text().length - 2)).split(", ");
  ...
  let listaPManutencao = $("#problemasManutencao").attr("title").substr(1, ($("#problemasManutencao").attr("title").length - 2)).split(", ");
  let listaFManutencao = $("#problemasManutencao").text().substr(1, ($("#problemasManutencao").text().length - 2)).split(", ");

  let contadorPagar = 0;
  let listaPPagar = $("#problemasPagar").attr("title").substr(1, ($("#problemasPagar").attr("title").length - 2)).split(", ");
  let listaFPagar = $("#problemasPagar").text().substr(1, ($("#problemasPagar").text().length - 2)).split(", ");
  ...
  $.each(teste, function(indice, valor){
    ...
    if ($('#statusProcesso' + $(valor).attr('id')).text() == "Limpeza"){
      listaPLimpeza[contadorLimpeza] > 0 ? $('#problemas' + $(valor).attr('id')).text(`${listaFLimpeza[contadorLimpeza]}/${listaPLimpeza[contadorLimpeza]}`) : null;
      contadorLimpeza += 1;
    } else if ($('#statusProcesso' + $(valor).attr('id')).text() == "Agendamento"){
      listaPAgendamento[contadorAgendamento] > 0 ? $('#problemas' + $(valor).attr('id')).text(`${listaFAgendamento[contadorAgendamento]}/${listaPAgendamento[contadorAgendamento]}`) : null;
      contadorAgendamento += 1;
    } else if ($('#statusProcesso' + $(valor).attr('id')).text() == "Manutenção"){
      listaPManutencao[contadorManutencao] > 0 ? $('#problemas' + $(valor).attr('id')).text(`${listaFManutencao[contadorManutencao]}/${listaPManutencao[contadorManutencao]}`) : null;
      contadorManutencao += 1;
    } else if ($('#statusProcesso' + $(valor).attr('id')).text() == "Pagar"){
      listaPPagar[contadorPagar] ? $('#problemas' + $(valor).attr('id')).text(`${listaFPagar[contadorPagar]}/${listaPPagar[contadorPagar]}`) : null;
      contadorPagar += 1;
    } else if ($('#statusProcesso' + $(valor).attr('id')).text() == "Concluído"){
      listaPConcluido[contadorConcluido] > 0 ? $('#problemas' + $(valor).attr('id')).text(`${listaFConcluido[contadorConcluido]}/${listaPConcluido[contadorConcluido]}`) : null;
      contadorConcluido += 1;
    } else {
    ...
    } else if ($("#stat" + $(valor).attr('id')).text() == 'A fazer'){
      if (inicial.getTime() >= dataAtual.getTime()){
  ...
  $(".titulos").html(`
    ...
    <span title="Adiantados: ${limpezaAdiantado}\nAvisos: ${limpezaAtencao}\nAtrasados: ${limpezaAtrasado}" class="titulo-child afazer">Limpeza | ${qtdProcessos[0].replace("[", "")}</span>
    <span title="Adiantados: ${manutencaoAdiantado}\nAvisos: ${manutencaoAtencao}\nAtrasados: ${manutencaoAtrasado}" class="titulo-child feito">Manutenção | ${qtdProcessos[2]}</span>
    <span title="Adiantados: ${pagarAdiantado}\nAvisos: ${pagarAtencao}\nAtrasados: ${pagarAtrasado}" class="titulo-child afazer">Pagar | ${qtdProcessos[3]}</span>
    <span title="Concluído: ${qtdProcessos[4]}" class="titulo-child fazendo">Concluído | ${qtdProcessos[4].replace("]", "")}</span>
    `);
