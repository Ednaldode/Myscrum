...
// Scripts double click para transportar tarefas entre os quadros ----------------------------
    //A fazer
    var touchtime = 0;
    $("#q-afazer").on("click", function() {
        if (touchtime == 0) {
            //Primeiro clique
            touchtime = new Date().getTime();
        } else {
            //Compara o tempo dos cliques para verificar duplo clique
            if (((new Date().getTime()) - touchtime) < 200) {
                //Caso seja duplo, executa a função e zera o contador
                $('#c-afazer').append($('.selecionada'));
                $('.tarefa').removeClass('selecionada');
                $('.tarefa').css("background-color","rgb(255, 255, 255)");
                touchtime = 0;
            } else {
                //Caso não seja duplo, salva o tempo do clique para comparar com o próximo
                touchtime = new Date().getTime();
            }
        }
    });
    ...
    //Fazendo
    var t_fazendo = 0;
    $("#q-fazendo").on("click", function() {
        if (t_fazendo == 0) {
            //Primeiro clique
            t_fazendo = new Date().getTime();
        } else {
            //Compara o tempo dos cliques para verificar duplo clique
            if (((new Date().getTime()) - t_fazendo) < 200) {
                //Caso seja duplo, executa a função e zera o contador
                $('#c-fazendo').append($('.selecionada'));
                $('.tarefa').removeClass('selecionada');
                $('.tarefa').css("background-color","rgb(255, 255, 255)");
                t_fazendo = 0;
            } else {
    ...
    //Feito
    var t_feito = 0;
    $("#q-feito").on("click", function() {
        if (t_feito == 0) {
            //Primeiro clique
            t_feito = new Date().getTime();
        } else {
            //Compara o tempo dos cliques para verificar duplo clique
            if (((new Date().getTime()) - t_feito) < 200) {
                //Caso seja duplo, executa a função e zera o contador
                $('#c-feito').append($('.selecionada'));
                $('.tarefa').removeClass('selecionada');
                $('.tarefa').css("background-color","rgb(255, 255, 255)");
                t_feito = 0;
            } else {
